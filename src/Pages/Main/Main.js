...
export default class Main extends Component {
  ...
  slickSlideLeft = () => {  //sectionfive 슬릭슬라이드
  ...
  slickSlideRight = () => { //sectionfive 슬릭슬라이드
    const {slidePage} = this.state;
    if(slidePage < 18){
      this.setState({
        slidePage : slidePage + 1,
  ...
  tabHandler = (e) => {  //sectionfour 탭
    this.setState({
      service : e.target.ariaLabel,
    })
  }
  mouseOver = (e) => {  //sectiontwo 마우스오버
    this.setState({
      day: e.target.name, //!계산된 속성명
    });
  };
  mouseOut = () => {   //sectiontwo 마우스아웃
    this.setState({
      day : "",
    });
  };
  componentDidMount() { 
    ...
    fetch('http://localhost:3000/data/data.json') //sectionfive 후기 데이터 불러오기
    .then(res => res.json())
    .then(res => {
      this.setState({
        reviewData : res["data"],
  ...
  intervalHandler = () => {
    setInterval(() => {   //sectionfour - textService 함수 반복 실행 함수
  ...
  transitionSlides = (count) => {   //sectionfour - textService 애니메이션 함수 
    let Arr = ["","","","",""];
